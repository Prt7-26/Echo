...
- ✅ **Step 23 (dashboard feedback loop made real + reason-LLM scoring)** — a live-testing pass on the web dashboard's `chat:bottom` rating widget surfaced (and fixed) several "green in tests, dead at runtime" gaps, then added LLM scoring of free-text reasons:
  ...
  - **Still-untested**: meaningful exclusion + its injection (only the degraded path was driven live); Layer A drift (needs ≥20 same-skill invocations to warm up); brand-new (non-resumed) conversation widget; multi-tab queue isolation; Tauri shell; setup-wizard live run.
