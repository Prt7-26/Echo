...
- ⬜ **Step 16+**: Open work — Tauri shell + clipboard IPC (proposal Phase 2), proposal §4 evaluation suite (datasets, simulated personas, baselines, metrics), upgrade Echo's encoder to a neural embedding via auxiliary.embedding config (would lift M1 condition 4 from lexical to true semantic).
