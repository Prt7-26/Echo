...
仓库中的大部分代码来自 Hermes，Echo 自身的代码主要集中在以下位置：
...
为了避免 agent 自我打分，评测采用了四个相互独立的模型——一个模拟用户、一个独立评分者、一个作为被测 agent，以及 Echo 自己的信号模型。评测对照了两个公开偏好基准（[PersonaMem](https://huggingface.co/datasets/bowen-upenn/PersonaMem) 和 [PrefEval](https://huggingface.co/datasets/siyanzhao/prefeval_explicit)），外加一个模拟用户闭环。完整的实验方法和数据——包括最初设定的开销目标在哪些方面未达成——请参阅 [`DevPlan/experiment-report-zh.md`](DevPlan/experiment-report-zh.md)（中文）和 [`experiment-report.md`](DevPlan/experiment-report.md)（English）。
